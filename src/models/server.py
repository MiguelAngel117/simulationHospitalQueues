...
class Server1:
    ...
    def process_patients(self, patients, queue_2, listRi):
        for patient in patients:
            ...
            print(f"{patient.name} llega a la cola 1 a las {patient.arrival_time:.3f}, empieza a ser atendido a las {patient.start_time:.3f} y termina a las {patient.end_time:.3f}, tiempo servicio {service_time:.3f}, tiempo espera {patient.waiting_time:.3f} \n")
...
class Server2:
    ...
    def process_patients(self, queue_2, queue_3, listRi):
        while True:
            ...
            print(f"{patient_name} llega a la cola 2 a las {end_time_1:.3f}, empieza a ser atendido por el servidor {self.server_id} a las {start_time_2:.3f} y termina a las {end_time_2:.3f}, tiempo servicio {service_time:.3f} \n")
            if random.random() < 0.25:
                queue_3.put((end_time_2, patient_name))
